...
export const GROUPS = {
  ...
  persona: { order: 2, title: '性格与人设', desc: '管它是什么性格、用什么语气、怎么说话。填了就注入，全留空 = 一个字都不改（保持现状）。' },
  notify: { order: 3, title: '通知', desc: '什么情况下手机会收到消息 —— 嫌吵就调这几个阈值。' },
  allow: { order: 4, title: '白名单（谁能跟它说话）', desc: '空白名单 + allowAllWhenEmpty 关闭 = 谁都不理。黑名单优先于白名单。' },
  deny: { order: 5, title: '黑名单', desc: '命中黑名单的会话一律不处理，优先级高于白名单。' },
  slang: { order: 6, title: '黑话学习', desc: '它会把聊天里看不懂的新词攒起来，够了就让 DSH 去研究并收录，之后聊天时会用上。' },
  social: { order: 7, title: '一代仿真（social）', desc: '早期模式：按概率决定要不要接话、要不要主动找人。' },
  socialV2: { order: 8, title: '二代仿真（socialV2）', desc: '现在实际在用的模式 —— AI 自己决定什么时候说话、什么时候潜水，这里是它的边界与配额。' },
  dsh: { order: 9, title: '模型与 DSH 接线', desc: '机器人跑在哪个模型上、连的是哪个 DSH。改完通常下一回合生效。' },
  snowluma: { order: 10, title: 'SnowLuma 接线（高级）', desc: 'QQ 客户端 ↔ 桥接之间的管道。地址填错会直接连不上，改完要重启桥接。' },
  security: { order: 11, title: '安全', desc: '拦截/越权时的行为。' }
};
...
export const FIELDS = [
  ...
  ['ownerQQ', 'int', '管理员 QQ',
    '填**你本人的 QQ 号**（不是机器人号）。桥接靠它判断"谁是主人"：只有这个号能批复权限、能收到审批转发；填错了你的消息会被当成普通用户处理。'],
  ...
  // ── 性格与人设 ────────────────────────────────────────────────────────
  // 这一段是「主人随手调性格」的入口：桥接会在每条消息的提示词最前面注入它，
  // 与 roles/<角色>.md 那种整张角色卡叠加（角色卡当前情背景、这里定当前性格语气）。
  // 全部留空 = 不注入（不填就不改它，见 src/bridge.js 的 personaBlock()）。
  ['persona.enabled', 'bool', '启用性格设置',
    '总开关。关掉 = 下面填的内容一个字都不注入提示词（内容还留着，方便随时切回来）。默认开；开着但每格都留空，效果与关掉完全一样。'],
  ['persona.name', 'string', '自称 / 名字',
    '它管自己叫什么（例如「小D」「诺诺」「小鲸鱼」）。填了提示词里会写明「你就叫这个名字」，自我介绍和自称都会用它；留空 = 沿用角色卡或 preset 里的名字。'],
  ['persona.personality', 'text', '性格（它是谁）',
    '写性格本身，不写要求。例如「慢热、嘴硬心软、被夸会害羞但装不在意；好奇心上头会一直追问」。越具体表现越稳；「要听话、要聪明」这种空话没用。留空 = 不改。'],
  ['persona.tone', 'text', '说话语气（什么调子）',
    '说话的口气与情绪底色，例如「随意直接、不堆敬语、不喊口号；偶尔犯懒，被逗会反驳」。管用什么调子说，不管说什么内容。留空 = 不改。'],
  ['persona.speechStyle', 'text', '说话方式（怎么组织句子）',
    '句子长短、整段还是分条、要不要反问、用不用表情与颜文字，例如「短句为主，一句话说完，不用 emoji，少用感叹号，不写长段落」。留空 = 不改。'],
  ['persona.habits', 'text', '口头禅与习惯',
    '反复出现的小习惯：口头禅、怎么称呼对方、遇到某类话题的固定反应，例如「叫你主人；同意时先说"行"；被问技术问题会认真展开、不糊弄」。留空 = 不改。'],
  ['persona.taboo', 'text', '禁忌（不许说 / 不许做）',
    '明确禁止的表达与行为，例如「不许自称 AI 助手、不许说"作为一个语言模型"、不用"亲"、不主动提自己是机器人」。这条优先级最高，和上面任何设定冲突时以它为准。留空 = 不改。'],
  ...
  ['relayQuestionsToOwner', 'bool', '把 DSH 提问转发到手机 QQ',
    '你在 DSH 界面里自己开的会话中，AI 用询问工具问你的**选项**会发到你的 QQ：回「1/2/3」选中对应选项、回选项原文也算选中、回别的文字就是自定义回答。多个问题用 | 分隔（例如 1|2）。问题正文与选项的详细说明都**完整发出、不截断**（换行也保留），手机上能看清每个方案到底是什么。**关掉后只有 QQ 会话里的提问会推给你**，DSH 界面里开的会话只能在电脑上答。默认开。'],
  ...
  ['socialV2.tools.getUnread', 'bool', '工具：qq_get_unread_messages（读未读）',
    '允许它读未读消息。关掉后它只能看见叫醒它的那一条，会显得很瞎。'],
  ...
  ['socialV2.tools.sendPrivate', 'bool', '工具：qq_send_private_message（发私聊）',
    '允许它给人发私聊。关掉 = 不能主动发消息（回复不受影响）。'],
  ['socialV2.tools.reply', 'bool', '工具：qq_reply（引用回复）',
    '允许它引用某条消息再回复 —— "回的是哪一句"写得清清楚楚。对方连着发好几条时很有用。'],
