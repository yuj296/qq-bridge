...
把 qq-bridge 的 **整份 `config.json`**（148 项，机密除外）搬进 DSH 设置页 ——
...
| 性格与人设 | 7 | 总开关、名字/自称、性格、说话语气、说话方式、口头禅与习惯、禁忌 —— 填了才注入提示词，全空 = 一个字都不改（见下） |
| 通知 | 7 | 任务完成通知的开关与阈值（默认只报跑够 5 分钟的）+ 审批 / 提问转发到手机 |
| 白名单 / 黑名单 | 1 / 1 | **私聊**准入（`allow.private` / `deny.private`）；群白/黑名单字段已删除 |
| 安全 | 1 | 拦截时是否通知 |
...
除 `social` 与 `socialV2` 这两个超长组默认收起（点标题展开）外，其余 9 组默认展开。

### 性格与人设（`persona.*`，2026-09-16 加）

改性格不用再去写整张角色卡：这 7 格填完保存，桥接 5 秒内就会把
`【性格设定】` 那段拼进给 agent 的提示词（`src/bridge.js` 的 `personaBlock()`）。
两条规矩：

- **留空 = 不注入**：六格全空（或关掉总开关）时，提示词与加这组设置之前**完全一致** ——
  不会因为升级就悄悄换了人设。
- **与角色卡叠加，越靠后越优先**：`roles/<角色>.md` 当底座（人设背景/世界观），
  这一段当「当前性格语气」，写进提示词时排在其后并注明冲突时以它为准，「禁忌」再压一层。

护栏：`node scripts/test-persona.mjs`（隔离实例 + 假 OneBot，21 项）。
...
node scripts/test-qq-settings-form.mjs   # 表单能不能真的改：输入汉字 / 勾选 / 清空 / 保存提交的值
```

`test-qq-settings-page.mjs` 用真 schema + react-dom/server 把页面渲染成 HTML，验：
注册进 `settings.section`、`id=qq-bot`、`order=1`、11 个分组都在、每组的"N 项"对得上、
默认展开的分组行数 = 44（148 项减去收起的 social 16 + socialV2 88）、字段说明真的渲染出来了。

`test-qq-settings-form.mjs` 补的是**渲染测不出来的那一半**：用 jsdom + react-dom 真挂载页面，
模拟输入 / 勾选 / 清空 / 选下拉 / 点「已改」/ 点保存（29 项）。起因是 2026-09-16 主人报
「设置里的内容无法进行更改、输入汉字无法显示」—— `FieldRow` 把草稿的**整条记录**当成了值给控件
（`value` 收到对象 → 框里显示 `[object Object]`、勾选框永远弹回），而 SSR 渲染一路全绿、
后端收到的值也一直是对的，光看渲染根本测不出来。同批还修了两处同源问题：下拉框选
「（不设置，沿用原值）」过去提交空串（enum 的 schema 只认常量，会被拒），现在提交 `unset`；
点「已改」撤销后过去仍显示旧覆盖值，现在显示回磁盘值。
**动 `lib/client.js` 的控件渲染时必须跑这支。**
