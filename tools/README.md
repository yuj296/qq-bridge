# tools/ —— 开机/登录自动启动（Windows 任务计划程序）—— **可选件，默认不装**

> ⚠️ **本目录是可选项，当前没有安装。** 机器人的正常启动方式是 DSH 侧边栏的
> **「唤醒」按键**（`plugins/qq-wake/`）：点一下才起来，不点就一直睡着。
> 只有你想要「开机自启 + 挂了自动重启」时，才跑下面的 `install-task.ps1` 把守护装上。
> 两者**可以共存**（唤醒按键发现进程已经活着就直接复用），但没必要。
